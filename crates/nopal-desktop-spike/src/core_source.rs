...
mod tests {
    ...
    fn production_runner_crosses_a_real_bounded_process_boundary() {
        ...
        let executable = directory.path().join("fake-nopal");
        fs::write(
            &executable,
            "#!/bin/sh\nprintf '%s' '{\"kind\":\"nopal.field/v1\",\"plots\":[],\"entries\":[]}'\n",
        )
        .expect("write fake nopal");
        let mut permissions = fs::metadata(&executable)
            .expect("fake nopal metadata")
            .permissions();
        permissions.set_mode(0o700);
        fs::set_permissions(&executable, permissions).expect("make fake nopal executable");

        let snapshot = super::CliCoreFieldSnapshotSource::production(executable)
            .with_output_limit(4096)
            .load_field_snapshot()
    ...
    fn production_runner_kills_a_query_that_exceeds_its_total_time_allowance() {
        ...
        let directory = tempfile::tempdir().expect("temporary executable directory");
        let executable = directory.path().join("slow-nopal");
        fs::write(&executable, "#!/bin/sh\nexec sleep 5\n").expect("write slow fake nopal");
        let mut permissions = fs::metadata(&executable)
            .expect("slow fake nopal metadata")
            .permissions();
        permissions.set_mode(0o700);
        fs::set_permissions(&executable, permissions).expect("make slow fake nopal executable");
        let started = Instant::now();

        let error = super::CliCoreFieldSnapshotSource::production(executable)
