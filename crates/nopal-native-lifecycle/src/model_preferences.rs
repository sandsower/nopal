...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn repeated_writes_keep_the_parent_and_replacement_private() {
        use std::os::unix::fs::PermissionsExt;

        let directory = tempdir().expect("create preference sandbox");
        let parent = directory.path().join("native");
        let path = parent.join("model-recents.json");
        let store = ModelRecentsStore::new(&path);

        assert_eq!(
            store.write(&[model("pi", "first")]).unwrap(),
            ModelRecentsWriteOutcome::Written
        );
        assert_eq!(
            store
                .write(&[model("pi", "second"), model("pi", "first")])
                .unwrap(),
            ModelRecentsWriteOutcome::Written
        );
        ...
        assert_eq!(
            store.read().unwrap(),
            ModelRecentsReadOutcome::Ready(vec![model("pi", "second"), model("pi", "first")])
        );
