...
Ambient user Git, curl, package-manager, Kubernetes, and related configuration is disabled or redirected into that empty home.
System Git configuration is disabled consistently for workspace observation, gate execution, and the protected Pi runtime because host-image configuration is outside the audited contract; user and repository Git configuration remains observed and bound by the workspace adapter.
