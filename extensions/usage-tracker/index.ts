...
import {
	...
	probeCopilotDirect,
	probeGoogleDirect,
	providerKeysForModel,
	probeOpenAIDirect,
	providerDisplayName,
	readPiAuth,
	shouldPreserveStaleWindows,
	tokenForProviderProbe,
...
export default function usageTracker(pi: ExtensionAPI) {
	...
	let activeCtxGeneration = 0;

	function clearActiveCtx(): void {
		activeCtx = null;
		activeCtxGeneration++;
	}

	function scheduleStatusLineUpdate(delayMs: number): void {
		const generation = activeCtxGeneration;
		setTimeout(() => {
			if (!activeCtx || generation !== activeCtxGeneration) return;
			try {
				const line = renderStatusLine(activeCtx, activeCtx.ui.theme);
				activeCtx.ui.setStatus("usage-tracker", line ?? undefined);
			} catch {
				// Contexts become stale after reload/session replacement. Delayed UI refreshes
				// are best-effort and must not crash headless child pi processes.
				clearActiveCtx();
			}
		}, delayMs);
	}
	...
	function normalizeProviderRateLimits(value: unknown): ProviderRateLimits | null {
		...
		if (!(candidate.provider === "anthropic" || candidate.provider === "openai" || candidate.provider === "google" || candidate.provider === "copilot")) {
			return null;
		}
	...
	async function probeProvider(provider: ProviderKey, force = false): Promise<void> {
		...
		try {
			...
			const probeToken = tokenForProviderProbe(provider, fresh.entry);
			if (!probeToken) {
				return;
			}
			...
			switch (provider) {
				case "anthropic":
					limits = await probeAnthropicDirect(probeToken);
					break;
				case "openai":
					limits = await probeOpenAIDirect(probeToken);
					break;
				case "google":
					limits = await probeGoogleDirect(probeToken, fresh.entry);
					break;
				case "copilot":
					limits = await probeCopilotDirect(probeToken);
					break;
			}
	...
	function triggerProbe(ctx: ExtensionContext, force = false): void {
		...
		for (const provider of providerKeysForModel(model.provider, model.id)) {
	...
	// ─── Status line rendering (footer, active provider only) ────────────

	function renderStatusLine(ctx: ExtensionContext, theme: { fg: (c: string, t: string) => string }): string | undefined {
		if (!widgetVisible || getSafeModeState().enabled) {
			return undefined;
		}

		// Determine which provider key(s) match the active model
		const model = ctx.model;
		if (!model) {
			return undefined;
		}
		const activeProviderKeys = providerKeysForModel(model.provider, model.id);

		// Find the best window among active-provider rate limit data
		let bestRl: (typeof rateLimits extends Map<unknown, infer V> ? V : never) | undefined;
		for (const key of activeProviderKeys) {
			const rl = rateLimits.get(key);
			if (rl && rl.windows.length > 0) {
				bestRl = rl;
				break;
			}
		}
		...
		const most = bestRl.windows.reduce((a, b) => (a.percentLeft < b.percentLeft ? a : b));
		const color = pctColor(most.percentLeft);
		const bar = theme.fg(color, progressBar(most.percentLeft, 6));
		const pct = theme.fg(color, `${most.percentLeft}%`);
		const reset = most.resetDescription ? theme.fg("dim", ` ↻${most.resetDescription}`) : "";
		const name = theme.fg("dim", providerDisplayName(bestRl.provider));
	...
	pi.on("session_switch", (_event, ctx) => {
		...
		hydrateFromSession(ctx);
		triggerProbe(ctx);
		ctx.ui.setStatus("usage-tracker", undefined);
	});
	...
	pi.on("turn_end", (event, ctx) => {
		...
		if (event.message.role === "assistant") {
			...
			// Update status line after probe fires (small delay to let probe complete)
			scheduleStatusLineUpdate(2500);
		}
	});
	...
	pi.on("model_select", (_event, ctx) => {
		...
		activeCtxGeneration++;
		triggerProbe(ctx);
		// Show cached data immediately if available, then repaint after fresh probe
		const cached = renderStatusLine(ctx, ctx.ui.theme);
		ctx.ui.setStatus("usage-tracker", cached ?? undefined);
		scheduleStatusLineUpdate(3000);
	});
	...
	pi.registerCommand("usage-toggle", {
		description: "Toggle the usage tracker status line visibility",
		async handler(_args, ctx) {
			widgetVisible = !widgetVisible;
			if (widgetVisible) {
				const line = renderStatusLine(ctx, ctx.ui.theme);
				ctx.ui.setStatus("usage-tracker", line ?? undefined);
				ctx.ui.notify("Usage status shown.", "info");
			} else {
				ctx.ui.setStatus("usage-tracker", undefined);
				ctx.ui.notify("Usage status hidden. Run /usage-toggle to show.", "info");
			}
		},
	});
