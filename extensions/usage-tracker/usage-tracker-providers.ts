...
function normalizeCopilotPlan(value: string | null): string | null {
	const normalized = value?.trim().toLowerCase();
	if (!normalized) {
		return null;
	}
	if (normalized === "pro_plus" || normalized === "pro-plus") {
		return "pro+";
	}
	return COPILOT_PLAN_LIMITS[normalized] ? normalized : null;
}

function resetDescriptionFromIso(value: string | null): string | null {
	if (!value) {
		return null;
	}
	const normalized = /^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T00:00:00.000Z` : value;
	return resetCountdown(normalized);
}

export function parseCopilotInternalUserQuota(payload: unknown): ProviderRateLimits {
	...
	const plan = normalizeCopilotPlan(
		firstString(payload, [["plan", "type"], ["plan", "name"], ["plan"], ["copilot_plan"], ["subscription_plan"], ["sku"]]),
	);
	result.plan = plan;

	let total = firstNumber(payload, [
		["quota", "limit"],
		["quota", "total"],
		["monthly_quota", "limit"],
		["monthly_quota", "total"],
	...
	const reset = firstString(payload, [
		["quota", "reset_at"],
	...
	if (total === null && plan) {
		total = COPILOT_PLAN_LIMITS[plan] ?? null;
	}

	if (total === null || total <= 0 || used === null || used < 0) {
		result.error = "GitHub Copilot usage response did not include usable quota fields.";
		return result;
	}

	const clampedUsed = Math.min(total, used);
	upsertWindow(result.windows, {
		label: `Premium requests (${clampedUsed}/${total} used)`,
		percentLeft: clampPercent(((total - clampedUsed) / total) * 100),
		resetDescription: resetDescriptionFromIso(reset),
		windowMinutes: null,
	});
...
export async function probeCopilotDirect(token: string, fetchImpl: FetchLike = fetch): Promise<ProviderRateLimits> {
	const result: ProviderRateLimits = {
		provider: "copilot",
		windows: [],
		credits: null,
	...
	try {
		const response = await fetchImpl(`${PROVIDER_API_BASE.copilot}/copilot_internal/user`, {
			method: "GET",
			headers: {
		...
		if (response.status === 401) {
			result.error = "GitHub Copilot auth token expired — re-authenticate in pi settings.";
			return result;
		}
		if (response.status === 429) {
			const retryAfter = Number.parseInt(response.headers.get("retry-after") ?? "", 10);
			const retryHint = Number.isFinite(retryAfter) ? ` (retry in ${fmtDuration(Math.max(0, retryAfter) * 1000)})` : "";
			result.note = `GitHub Copilot usage endpoint is rate-limited${retryHint}.`;
			return result;
		}
		if (!response.ok) {
			result.note = `GitHub Copilot usage endpoint returned ${response.status} — quota details unavailable.`;
			return result;
		}

		return parseCopilotInternalUserQuota(await response.json());
	} catch (e) {
		if (e instanceof Error && e.name === "TimeoutError") {
			result.error = "GitHub Copilot API probe timed out";
		} else {
...
export function tokenForProviderProbe(
	provider: ProviderKey,
	tokens: Pick<PiAuthEntry, "access" | "refresh">,
): string | null {
	if (provider === "copilot") {
		return tokens.refresh || tokens.access || null;
	}
	return tokens.access || null;
}

export function providerKeysForModel(provider: string | undefined, modelId: string): ProviderKey[] {
